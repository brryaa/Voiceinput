...
class GeminiApiClient {
    ...
    suspend fun getAiCandidates(
        ...
        if (pinyin.isBlank() && context.isBlank()) return Result.success(emptyList())
    ...
    private fun buildPrompt(pinyin: String, context: String): String {
        return if (pinyin.isBlank()) {
            "上下文：「${context}」\n请根据上下文进行字词联想，预测接下来最可能输入的词。"
...
如果是字词联想功能（没有提供拼音，只有上下文），请预测接下来用户最可能输入的紧跟在这个词后面的候选词。
